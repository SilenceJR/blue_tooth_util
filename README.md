...
`0x0112` 通过 `queryPrayerReminder()`、`setPrayerReminder(config)`、
`disablePrayerReminder()` 查询、开启设置及关闭。`RingPrayerReminder` 明确起止小时、
分钟和 `intervalMinutes`，完整保留分钟精度。关闭仅发送 `[00]`，设备保留时段；
配置校验不施加 App 整点 UI 的动态间隔限制。命令串行，App 写后查询对账。
详见[诵经提醒协议及金标准帧](docs/戒指_诵经提醒_App对接文档.md)。

App 负责时间同步、能力提示、写后回读和整点 UI；明确未知命令才提示不支持，
超时需要重查。V0.16 文件名不证明设备已升级或实际到点提醒通过。
